...
enum Color { UNCOLORED = -1, RED = 0, BLACK = 1 };
...
public:
    ...
    bool isBipartite(int startVertexID){
        ...
        for (auto& vertex : vertices) {
            color[vertex.state_id] = UNCOLORED;
        }

        queue<int> bfsQueue;
        color[startVertexID] = RED;
        ...
        while (!bfsQueue.empty()) {
            ...
            if (currentVertex != nullptr) {
                ...
                for (auto& edge : currentVertex->edgeList) {
                    ...
                    if(color[neighborID] == UNCOLORED){
                        color[neighborID] = 1 - color[currentVertexID];
                        bfsQueue.push(neighborID);
                    }

                    else if(color[neighborID] == color[currentVertexID])
                    {
                        return false;
                    }
