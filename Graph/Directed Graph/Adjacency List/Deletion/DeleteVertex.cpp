...
public:
    ...
    void deleteVertexByID(int vid) {
        int vIndex = -1;  // Initialize vIndex to -1 to indicate vertex not found
        ...
        if (vIndex == -1) {
            cout << "Vertex with ID " << vid << " not found." << endl;
            return;
        }
        ...
        for (int i = 0; i < vertices.size(); i++) {
            for (auto it = vertices.at(i).edgeList.begin(); it != vertices.at(i).edgeList.end(); ) {
                if (it->getDestinationVertexID() == vid) {
                    it = vertices.at(i).edgeList.erase(it); 
                } else {
                    ++it;  
                }
            }
        ...
        cout << "Vertex with ID " << vid << " deleted successfully." << endl;
    }
