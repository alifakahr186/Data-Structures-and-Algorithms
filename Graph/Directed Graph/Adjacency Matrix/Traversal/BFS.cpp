...
public:
    ...
    Vertex* getVertexByID(int vid) {
        int index = getVertexIndexByID(vid);
        if (index != -1) {
    ...
    void addEdgeByID(int fromVertex, int toVertex, int weight) {
        ...
        // Add the edge in one direction for a directed graph
    ...
    void BFS(int startVertexID) {
        ...
        while (!bfsQueue.empty()) {
            ...
            if (currentVertex != nullptr) {
                ...
                int currentIndex = getVertexIndexByID(currentVertexID);
                for (int i = 0; i < vertices.size(); ++i) {
                    if (adjMatrix[currentIndex][i] != 0 && !visited[vertices[i].getStateID()]) {
                        visited[vertices[i].getStateID()] = true;
                        bfsQueue.push(vertices[i].getStateID());   
                    }
                }
            }
        }

        cout << endl;  // Finish BFS traversal
    }
