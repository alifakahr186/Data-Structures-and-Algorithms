...
public:
    ...
    void DFS(int startVertexID) {
        ...
        while (!dfsStack.empty()) {
            ...
            Vertex* currentVertex = &vertices[getVertexIndexByID(currentVertexID)];
            cout << currentVertex->getStateName() << " ";  

            // Visit all neighbors of the current vertex
            for (int i = 0; i < vertices.size(); ++i) {
                if (adjMatrix[getVertexIndexByID(currentVertexID)][i] != 0 && !visited[vertices[i].getStateID()]) {
                    visited[vertices[i].getStateID()] = true;
                    dfsStack.push(vertices[i].getStateID());
                }
            }
        }
