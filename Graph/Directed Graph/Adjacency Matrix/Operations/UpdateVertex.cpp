...
public:
    ...
    void updateVertexByID(int oldVID, const string& vname) {
        int vertexIndex = getVertexIndexByID(oldVID);
        if (vertexIndex == -1) {
            cout << "Vertex with ID " << oldVID << " does not exist.\n";
            return;
        }

        vertices[vertexIndex].setStateName(vname);
        cout << "Vertex(State) Updated Successfully\n";
    }
...
int main() {
    ...
    do {
        ...
        case 3:
            id1 = getValidIntInput("Enter State ID of Source Vertex: ");
            sname = getValidStringInput("Enter State Name: ");
            graph.updateVertexByID(id1, sname);
            break;
