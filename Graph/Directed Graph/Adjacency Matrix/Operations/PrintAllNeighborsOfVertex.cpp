...
public:
    ...
    void addEdgeByID(int fromVertex, int toVertex, int weight) {
        ...
        // Add the edge in one directions for an directed graph
    ...
    void getAllNeigborsByID(int vid) {
        int vertexIndex = getVertexIndexByID(vid);
        if (vertexIndex == -1) {
            cout << "Vertex with ID " << vid << " does not exist.\n";
            return;
        }

        cout << vertices[vertexIndex].getStateName() << " (" << vertices[vertexIndex].getStateID() << ") --> ";
        
        bool foundNeighbors = false;
        for (int i = 0; i < vertices.size(); ++i) {
            if (adjMatrix[vertexIndex][i] != 0) {  // If there is an edge
        ...
        if (!foundNeighbors) {
            ...
            cout << "\b\b ";  // Removes the last arrow
            cout << "\n";
        }
    }
...
int main() {
    ...
    do {
        ...
        cout << "1. Add Vertex\n";
        cout << "2. Add Edge\n";
        cout << "3. Print All Neigbors of a Vertex" << endl;
        cout << "4. Print Graph\n";
        cout << "0. Exit Program\n";
