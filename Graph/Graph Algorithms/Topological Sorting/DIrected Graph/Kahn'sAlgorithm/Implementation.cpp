    ...
    private:
        bool isCyclicUtil(int v, vector<bool> &visited, vector<bool> &recStack) {
            if (!visited[v]) {
                ...
                int vertId = vertices[v].state_id;
        ...
        void kahn() {
            ...
            for (auto &vertex : vertices) {
                for (auto &edge : vertex.getEdgeList()) {
                    int destIndex = idToIndex[edge.getDestinationVertexID()]; 
                    indegree[destIndex]++;
                }
            }
            ...
            while (!q.empty()) {
                int u = q.front();
                q.pop();
                cout << vertices[u].getStateID() << " ";

                for (auto &edge : vertices[u].getEdgeList()) {
                    int destIndex = idToIndex[edge.getDestinationVertexID()];
                    if (--indegree[destIndex] == 0) {
                        q.push(destIndex);
                    }
                }
            }
...
int main() {
    ...
    do {
        ...
        cout << "3. Kahn's Algorithm For Topological Sort" << endl;
        cout << "4. rint Graph" << endl;
        ...
        case 3:
            cout << "Kahn's Algorithm For Topological Sort" << endl;
            if (!g.isCyclic()) {
                g.kahn();
            } else {
                cout << "Kahn's Algorithm is not possible: The graph contains a cycle." << endl;
            }
            break;
