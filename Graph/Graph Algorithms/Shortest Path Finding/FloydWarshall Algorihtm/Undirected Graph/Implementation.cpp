...
public:
    ...
    void addVertex(const Vertex& newVertex) {
        ...
        for (auto& row : adjMatrix) {
            row.resize(newSize, INT_MAX); // Initialize new elements to INT_MAX
        }
    ...
    void addEdgeByID(int fromVertex, int toVertex, int weight) {
        ...
        adjMatrix[toIndex][fromIndex] = weight;  // Add this line to make the graph undirected
    ...
    void FloydWarshall(){
        ...
        for (int dist = 0; dist < vertices.size(); ++dist) {
            for (int i = 0; i < vertices.size(); ++i) {
                for (int j = 0; j < vertices.size(); ++j) {
                    if (distance[i][dist] != INT_MAX && distance[dist][j] != INT_MAX && 
                        distance[i][j] > distance[i][dist] + distance[dist][j]) {
    ...
    void printGraph() {
        ...
        for (int i = 0; i < vertices.size(); ++i) {
            ...
            for (int j = 0; j < vertices.size(); ++j) {
                // Print edge weights; INT_MAX indicates no edge
...
int main() {
    ...
    do {
        ...
        cout << "3. Calculate Single Shortest Path Using Floyd Warshall Algorithm " << endl;
        ...
        switch (option) {
            ...
            case 3:
                cout << "Calculate Single Shortest Path Using Floyd Warshall Algorithm " << endl;
                g.FloydWarshall();
                break;
