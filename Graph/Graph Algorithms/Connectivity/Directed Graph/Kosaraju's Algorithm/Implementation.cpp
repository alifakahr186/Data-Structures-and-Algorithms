...
class Graph {
    ...
    void DFSTraveral(int v, vector<bool> &visited, stack<int> &Stack) {
        visited[v] = true;

        for (Edge &edge : vertices[i].edgeList) {
            if (!visited[edge.getDestinationVertexID()]) {
                DFSTraveral(edge.getDestinationVertexID(), visited, Stack);
            }
        }
        Stack.push(v);
    }
    ...
    void DFSUtil(int i, vector<bool> &visited) {
        ...
        for (Edge &edge : vertices[i].edgeList) {
            if (!visited[edge.getDestinationVertexID()]) {
                DFSUtil(edge.getDestinationVertexID(), visited);
            }
        }
    }
    ...
    Graph getTranspose() {
        ...
        for (int v = 0; v < vertices.size(); v++) {
            for (Edge &edge : vertices[v].edgeList) {
                g.vertices[edge.getDestinationVertexID()].edgeList.push_back(Edge(v, edge.getWeight()));
            }
        }
        return g;
    }
...
int main() {
    ...
    do {
        ...
        cout << "3. Kosaraju's Algorithm For Strongly Connected Component:" << endl;
        ...
        switch (option) { 
            ...
            case 3:
                cout << "Kosaraju's Algorithm For Strongly Connected Component:" << endl;
                g.kosaraju();
                break;
