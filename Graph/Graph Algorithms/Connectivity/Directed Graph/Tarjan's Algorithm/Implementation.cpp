...
class Graph {
    ...
    void DFSTraveral(int v, vector<bool> &visited, vector<bool> &inStack, vector<int> &low_link, stack<int> &Stack) {
        static int id_counter = 0;
        visited[v] = true;
        low_link[v] = id_counter;
        id_counter++;

        inStack[v] = true;
        Stack.push(v);

        for (Edge &edge : vertices[v].edgeList) {
            if (!visited[edge.getDestinationVertexID()]) {
                DFSTraveral(edge.getDestinationVertexID(), visited, inStack, low_link, Stack);
            }
            if (inStack[edge.getDestinationVertexID()]) {
                low_link[v] = min(low_link[v], low_link[edge.getDestinationVertexID()]);
            }
        }
    ...
    void tarjan() {
        ...
        vector<int> low_link(vertices.size(), 0);

        for (int i = 0; i < vertices.size(); i++) {
            if (!visited[i]) {
                DFSTraveral(i, visited, inStack, low_link, Stack);
            }
        }
    }
...
int main() {
    ...
    do {
        ...
        cout << "3. Tarjan's Algorithm For Strongly Connected Component:" << endl;
        ...
        case 3:
            cout << "Tarjan's Algorithm For Strongly Connected Component:" << endl;
            g.tarjan();
            break;
