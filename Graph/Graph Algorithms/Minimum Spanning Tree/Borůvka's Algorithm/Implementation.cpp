...
public:
    ...
    vector<tuple<int, int, int>> convertToEdgeList() {
        ...
        for (auto vertex : vertices) {
            ...
            for (auto edge : vertex.edgeList) {
                ...
                if ((toVertexID, fromVertexID, weight) not in edgeList && (fromVertexID, toVertexID, weight) not in edgeList) {
                    edgeList.push_back(make_tuple(fromVertexID, toVertexID, weight));
                }
            }
        }
        return edgeList;
        ...
        while (numTrees > 1) {
            ...
            for (int i = 0; i < vertices.size(); i++) {
                for (auto edge : vertices[i].edgeList) {
                    ...
                    int v = findParent(parent, edge.DestinationVertexID);
                    if (u != v) {
                        if (cheapest[u] == -1 || edge.weight < vertices[cheapest[u]].edgeList.front().weight) {
                            cheapest[u] = edge.DestinationVertexID;
                        }
                        if (cheapest[v] == -1 || edge.weight < vertices[cheapest[v]].edgeList.front().weight) {
                            cheapest[v] = i;
                        }
                    } 
                }
            ...
            for (int i = 0; i < vertices.size(); i++) {
                if (cheapest[i] != -1) {
                    int u = findParent(parent, i);
                    int v = findParent(parent, cheapest[i]);
                    if (u != v) {
                        minWeight += vertices[i].edgeList.front().weight;
...
int main() {
    ...
    do {
        ...
        cout << "4. Calculate Minimum Spanning Tree using Boruvka's Algorithm" << endl;
        ...
        switch (option) {
            ...
            case 4:
                cout << "Calculate Minimum Spanning Tree using Boruvka's Algorithm -" << endl;
                cout << "Minimum Weight of Spanning Tree: " << g.boruvka() << endl;
                break;
