...
class Graph {
  ...
  void addEdgeByID(int fromVertex, int toVertex, int weight) {
   ...
   void BFSTraversal(int startVertexID) {
    ...
    vector<int> visited(vertices.size(), 0); // Initialize all vertices as unvisited
    queue<int> bfsQueue;

    bfsQueue.push(startVertexID);

    cout << "Breadth-First Search Traversal starting from Vertex " << getVertexByID(startVertexID).getStateName() << ": ";

    while (!bfsQueue.empty()) {
        int currentVertexID = bfsQueue.front();
        bfsQueue.pop();

        visited[currentVertexID - 1] = 1; 

        Vertex currentVertex = getVertexByID(currentVertexID);
        cout << currentVertex.getStateName() << " ";

        list<Edge> edges = currentVertex.getEdgeList();

        for (auto it = edges.begin(); it != edges.end(); it++) {
            int neighborID = it->getDestinationVertexID();
            if (!visited[neighborID - 1]) {
                parent[neighborID - 1] = currentVertexID; 
                bfsQueue.push(neighborID);
            }
            
            else if (parent[currentVertexID - 1] != neighborID) {
                cout << "\nCycle detected at edge " << currentVertex.getStateName() << " - " 
                << getVertexByID(neighborID).getStateName() << endl;
                return;
            }
        }
    }
...
int main() {
  ...
  do {
    ...
    cout << "3. Cycle Detection Using BFS Traversal" << endl;
    ...
    case 3:
        cout << "Bredth-First Search Based Cycle Detection Operation -" << endl;
        cout << "Enter ID of Starting Vertex(State): ";
        cin >> id1;
        if (g.BFSTraversal(id1)) {
