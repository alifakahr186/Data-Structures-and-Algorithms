        ...
        for (int i = 0; i < vertices.size(); i++)
        {
            if (!visited[i])
            {
                ...
                while (!q.empty())
                {
                    ...
                    for (auto& edge : vertices[v].edgeList)
                    {
                        int dest = edge.getDestinationVertexID();
                        if (!visited[dest])
                        {
                            visited[dest] = true;
                            q.push(dest);
                        ...
                        else if (inQueue[dest])
                        {
                            return true; //In this part cycle detected
                        }
...
int main() {
    ...
    do {
        ...
        case 3:
            cout << "Cycle Detection Using BFS Traversal -" << endl;
