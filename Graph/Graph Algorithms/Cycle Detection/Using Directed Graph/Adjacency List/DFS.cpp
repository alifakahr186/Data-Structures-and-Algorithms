...
public:
    ...
    bool detectCycleUsingDFSUtil(int vertexID, unordered_map<int, bool>& visited, unordered_map<int, bool>& recStack) {
        ...
        for (auto& edge : currentVertex->edgeList) {
            ...
            if (!visited[neighborID] && detectCycleUsingDFSUtil(neighborID, visited, recStack)) {
                return true;
            } else if (recStack[neighborID]) {
                cout << "Cycle detected involving vertex " 
                    << currentVertex->getStateName() 
                    << " (" << currentVertex->getStateID() 
                    << ") and "
                    << getVertexByID(neighborID)->getStateName() 
                    << " (" << neighborID << ")" 
                    << endl;
                return true;
            }
        ...
        recStack[vertexID] = false; // Remove vertex from recursion stack
        return false;
    }
    ...
    void detectCycleUsingDFS(int startVertexID) {
        ...
        // Detect cycle starting from each unvisited vertex
        for (auto& vertex : vertices) {
            int vertexID = vertex.getStateID();
            if (!visited[vertexID] && detectCycleUsingDFSUtil(vertexID, visited, recStack)) {
                return; // Cycle found, no need to continue
            }
        }
