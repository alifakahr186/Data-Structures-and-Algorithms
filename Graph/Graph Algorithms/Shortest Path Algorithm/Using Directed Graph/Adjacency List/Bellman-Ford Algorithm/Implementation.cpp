...
public:
    ...
    void BellmanFord(int startVertexID, unordered_map<int, int>& distance) {
        ...
        for (int i = 0; i < V - 1; i++) {
            for (auto& vertex : vertices) {
                int u = vertex.getStateID();
                if (distance[u] == INT_MAX) continue;  // Skip if u is unreachable
                ...
                for (Edge &edge : vertex.edgeList) {
                    ...
                    // Relax the edge
                    if (distance[u] + weight < distance[neighborID]) {
    ...
    void findShortestPath(int startVertexID, int endVertexID) {
        ...
        BellmanFord(startVertexID, distance);
...
int main() {
    ...
    do {
        ...
        cout << "1. Add Vertex\n";
        cout << "2. Add Edge\n";
        cout << "3. Calculate Single Shortest Path Using Bellman Ford Algorithm\n";
        cout << "4. Print Graph\n";
        cout << "0. Exit Program\n";
