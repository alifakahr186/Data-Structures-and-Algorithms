...
public:
    ...
    void DFS(int vertexID, unordered_map<int, bool>& visited, stack<int>& Stack) {
        visited[vertexID] = true;

        for (auto& edge : getVertexByID(vertexID)->edgeList) {
    ...
    Graph getTranspose() {
        ...
        for (auto& vertex : vertices) {
            transposedGraph.addVertex(Vertex(vertex.getStateID(), vertex.getStateName()));
        }
        ...
        for (auto& vertex : vertices) {
            for (auto& edge : vertex.edgeList) {
                int neighborID = edge.getDestinationVertexID();
                transposedGraph.addEdgeByID(neighborID, vertex.getStateID());
            }
        }
    ...
    void kosaraju() {
        ...
        for (auto& vertex : vertices) {
            if (!visited[vertex.getStateID()]) {
                DFS(vertex.getStateID(), visited, Stack);
            }
        }
...
int main() {
    ...
    do {
        ...
        cout << "1. Add Vertex\n";
        cout << "2. Add Edge\n";
        cout << "3. Strongly Connected Components Using kosaraju's Algorithm\n";
        cout << "4. Print Graph\n";
        cout << "0. Exit Program\n";
