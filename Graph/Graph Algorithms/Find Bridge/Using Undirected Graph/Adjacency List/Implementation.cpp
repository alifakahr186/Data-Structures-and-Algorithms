...
class FindBridges : public Graph {
...
private:
    void DFS(int vertexID) {
        ...
        startTime[vertexID] = finishTime[vertexID] = ++time;
        ...
        for (auto &edge : currentVertex->edgeList) {
            ...
            if (!visited[neighborID]) {
                ...
                finishTime[vertexID] = min(finishTime[vertexID], finishTime[neighborID]);

                if (finishTime[neighborID] > startTime[vertexID]) {
                    cout << "\nBridge found between " << vertexID << " and " << neighborID << endl;
                    bridges.push_back({vertexID, neighborID});

                }
            }

            else if (neighborID != parent[vertexID]) {
                finishTime[vertexID] = min(finishTime[vertexID], startTime[neighborID]);
            }
        }
    }

public:
    void findBridges(int startVertexID){
...
int main() {
    ...
    do {
        ...
        cout << "1. Add Vertex\n";
        cout << "2. Add Edge\n";
        cout << "3. Find Bridges In A Graph Using DFS\n";
        cout << "4. Print Graph\n";
        cout << "0. Exit Program\n";
